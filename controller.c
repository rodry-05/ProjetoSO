...
int main(int argc, char *argv[]) {
    ...
    int user_executions[MAX_RUNNERS] = {0}; // index is user_id, value is number of executions ; index 0 is total executions
    ...
    while (1) {        
        if (read(fd_requests, &request, sizeof(Message)) > 0) {
            ...
            if (request.type == 1) { // execute command
                ...
                for (int i = 0; i < number_of_running_commands; i++) {
                    if (running_commands[i].pid == request.pid) {
                        ...
                        struct timeval start = running_commands[i].start_time;

                        int user_id = running_commands[i].user_id;
                        user_executions[user_id]++;
                        user_executions[0]++; // total executions
                        ...
                        double duration = (end_time.tv_sec - start.tv_sec) + (end_time.tv_usec - start.tv_usec) / 1000000.0;
            ...
            // Least Executed First (LEF)
            } else if (strcmp(policy, "lef") == 0) {
                while (number_of_running_commands < parallel && number_of_scheduled_commands > 0) {

                    int idx = pick_next(scheduled_commands, number_of_scheduled_commands, user_executions);
                    Message next = scheduled_commands[idx];

                    for (int i = idx; i < number_of_scheduled_commands - 1; i++) {
                        scheduled_commands[i] = scheduled_commands[i + 1];  // shift the remaining commands to fill the gap
                    }

                    number_of_scheduled_commands--;
                    running_commands[number_of_running_commands++] = next;
