#!/bin/bash
# Test: ./runner -e 1 "echo texto | cat > ficheiro"
# Checks: pipe + stdout redirect combined — ficheiro contains "texto"
#
...
OUTPUT=$(./runner -e 1 "echo texto | cat > ficheiro" 2>&1)
RUNNER_EXIT=$?
sleep 0.2
...
cat "$OUTPUT_FILE" 2>/dev/null || echo "(file missing)"
echo "-------------------------"
...
# 4. "texto" did not appear in terminal output (it went to ficheiro, not stdout)
if echo "$OUTPUT" | grep -qx "texto"; then
    fail "'texto' leaked to terminal (should be in ficheiro only)"
else
    pass "'texto' did not leak to terminal"
fi
